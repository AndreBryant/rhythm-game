...
local Chunk = require("lib.luamidi.LuaMidi.Chunk")
...
function Writer.new(tracks)
	assert(
		type(tracks) == "table" and (tracks.type or tracks[1].type),
		"'tracks' must be a Track object or array of Track objects"
	)
...
function Writer:add_tracks(new_tracks)
	assert(
		type(new_tracks) == "table" and (new_tracks.type or new_tracks[1].type),
		"'new_tracks' must be a Track object or array of Track objects"
	)
