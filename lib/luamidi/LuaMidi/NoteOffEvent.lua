...
function NoteOffEvent.new(fields)
	...
	if self.timestamp ~= nil then
		assert(
			type(self.timestamp) == "number" and self.timestamp >= 0,
			"'timestamp' must be a positive integer representing the explicit number of ticks"
		)
	else
		self.timestamp = 0
	end
...
function NoteOffEvent:set_timestamp(timestamp)
	assert(
		type(timestamp) == "number" and timestamp >= 0,
		"'timestamp' must be a positive integer representing the explicit number of ticks"
	)
