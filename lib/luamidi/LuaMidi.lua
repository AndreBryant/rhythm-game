...
function LuaMidi.get_MIDI_tracks(path)
	...
	if MIDI then
		...
		for i = 1, #buffer do
			...
			if buffer[i] == 0x00 and buffer[i + 1] == 0xFF and buffer[i + 2] == 0x2F and buffer[i + 3] == 0x00 then
				append = false
			end
			if append then
				...
				if raw_track[i] == 0xFF then -- METADATA
					...
					elseif raw_track[i + 1] == LuaMidi.Constants.META_KEY_SIGNATURE_ID then
						...
						converted_data = converted_data
							.. " ("
							.. keys[sharps_num + 1][raw_track[i + 4] + 1]
							.. " "
							.. majmin[raw_track[i + 4] + 1]
							.. ")"
					end
...
function LuaMidi.add_tracks_to_MIDI(input, tracks, output)
	...
	if MIDI then
		...
		MIDI:close()
		if not (buffer[1] == 0x4D and buffer[2] == 0x54 and buffer[3] == 0x68 and buffer[4] == 0x64) then
			return false
		end
