...
function NoteEvent.new(fields)
	assert(
		type(fields.pitch) == "string" or type(fields.pitch) == "number" or type(fields.pitch) == "table",
		"'pitch' must be a string, a number or an array"
	)
	...
	if self.duration ~= nil then
		assert(
			type(self.duration) == "string" or (type(self.duration) == "number" and self.duration >= 0),
	...
	if self.velocity ~= nil then
		assert(
			type(self.velocity) == "number" and self.velocity >= 0 and self.velocity <= 100,
			"'velocity' must be an integer from 0 to 100"
		)
	else
		self.velocity = 50
	end
	...
	if self.channel ~= nil then
		assert(
			type(self.channel) == "number" and self.channel >= 1 and self.channel <= 16,
			"'channel' must be an integer from 1 to 16"
		)
	else
		self.channel = 1
	end
...
function NoteEvent:set_pitch(pitch)
	assert(
		type(pitch) == "string" or type(pitch) == "number" or type(pitch) == "table",
		"'pitch' must be a string, a number or an array"
	)
...
function NoteEvent:set_velocity(velocity)
	assert(
		type(velocity) == "number" and velocity >= 0 and velocity <= 100,
		"'velocity' must be an integer from 0 to 100"
	)
